...
// solve_task4_file:
//   N       = numar de noduri în graf
//   adj     = matricea de adiacenta NxN
//   coords  = perechi (row,col) pentru fiecare nod 
//   out     = fisierul de iesire
void solve_task4_file(int N, int adj[][100], int coords[][2], FILE *out);

// traverse_hamilton:
//   parcurge arborele în preordine si la fiecare nod
//   reconstruit curent in generatia depth,
//   construieste graful si ruleaza Task 4
void traverse_hamilton(struct Tree *root, char **cur, int n, int m, int depth, int K, FILE *out);
#endif
